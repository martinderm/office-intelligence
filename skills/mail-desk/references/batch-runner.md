...
Der Runner bleibt während der modularen Migration Eigentümer von CLI, Konfiguration, Dispatch und dem kanonischen Ergebnis-Envelope. Sechs Handler liegen unter `scripts/core/modes/`: `search.py` enthält `run_search_mode`, `resolve.py` enthält `run_resolve_mode`, `inspect.py` enthält `run_inspect_mode`, `draft.py` enthält `run_draft_mode`, `sync_sent.py` enthält `run_sync_sent_mode` und `verify.py` enthält `run_verify_mode`. `search` und `resolve` werden direkt importiert und re-exportiert. Für `inspect`, `draft`, `sync_sent` und `verify` behält der Runner schlanke gleichnamige Kompatibilitäts-Fassaden, die seine bisherigen patchbaren Abhängigkeiten zur Laufzeit einspeisen. Damit bleiben bestehende Imports, Patches, Modus-Aliase sowie Cleanup-, Manifest-, Sent-Index- und Konsistenzprüf-Semantik stabil, ohne einen Importzyklus zu erzeugen. Nur `execute` und `pipeline` verbleiben im Runner, der aktuell 1.224 physische Zeilen umfasst.
