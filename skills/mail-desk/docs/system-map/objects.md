...
* **Finales `attachment_evaluation` im DraftManifest (FR-15 / MD-E2-T01/T02/T03)** ([`scripts/core/attachment_reclassification.py`](../scripts/core/attachment_reclassification.py)):
  ...
  * `classifier_revision = compute_classifier_revision(rules_fingerprint, sortierte konsumierte Hashes)`; `rules_fingerprint` bindet den normalisierten AST des Classifier-Regelmoduls `classifier.py` plus die aktiven Projekt-/Topic-Kataloge und `CLASSIFIER_RULES_VERSION`. Reihenfolge ist irrelevant; Code-Regel- und Katalog-Änderungen bewegen die Revision, Kommentare/Formatierung/Host-Pfade nicht. Caller-, Mail- und Manifest-Werte können die Revision nicht setzen.
