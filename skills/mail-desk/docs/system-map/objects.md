...
* **Staged `attachment_evaluation` (FR-15 / MD-E1-T04)** ([`scripts/core/attachment_evaluation.py`](../scripts/core/attachment_evaluation.py)):
  * Kanonisches, staged Zwischenergebnis des öffentlichen `attachment_evaluate`-Orchestrators. Der Rückgabe-Envelope ist exakt `{"attachment_evaluation": {…}}`; T05 kann später ein Sibling `attachment_analysis_handoff` ergänzen.
  * Felder: `status ∈ {completed, not_needed, skipped, failed}` (beschreibt nur die Evaluierungsstufe), ein bounded `reason` (`classification_clear`, `no_attachments`, `no_allowed_attachments`, `evaluation_pending` in T04), `authorization ∈ {auto_evaluated, not_applicable}`, ein bounded `files[]`, `used_for_classification` **immer** `false`, `classifier_revision` **immer** `null`.
  * `attachment_evaluate` akzeptiert nur rohes MIME plus trusted Message-/Binding-Kontext und Decision-/Read-Escalation-Metadaten; Kandidaten-, Policy-, Fetch- oder Receipt-Werte des Callers sind keine Autorität. `inventory_sha256` wird aus dem revalidierten MIME-Kandidaten-SHA-256 abgeleitet (MD-A2-Fetch-Semantik), der kanonische `review_hash` via `compute_review_hash` gebildet und die interne Maschinen-Autorisierung (T03) gemint und sofort im `evaluation`-Kontext geprüft.
  * **T04 ist ein Skeleton:** `files[]` bleibt leer, `status: "skipped"` / `reason: "evaluation_pending"` / `authorization: "auto_evaluated"` ist der ehrliche Pre-T05-Ausgang. Fetch, Extraktion und Handoff (`op_attachment_fetch`, `extract_attachment_content`, `build_attachment_analysis_handoff`) sowie jede `DraftManifest`-Installation sind **MD-E1-T05** und hier nicht implementiert.
