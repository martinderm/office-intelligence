...
* **Staged `attachment_evaluation` + validierter `attachment_analysis_handoff` (FR-15 / MD-E1-T04/T05)** ([`scripts/core/attachment_evaluation.py`](../scripts/core/attachment_evaluation.py)):
  * Kanonisches, staged Zwischenergebnis des öffentlichen `attachment_evaluate`-Orchestrators. Der Rückgabe-Envelope ist exakt `{"attachment_evaluation": {…}, "attachment_analysis_handoff": {…}}`; der zweite Schlüssel trägt den mit `validate_attachment_handoff` validierten, gekapselten Handoff.
  * Felder des staged Objekts: `status ∈ {completed, not_needed, skipped, failed}` (beschreibt nur die Evaluierungsstufe), ein bounded `reason` (T05-Laufzeit: `classification_clear`, `no_attachments`, `no_allowed_attachments`, `handoff_ready`, `still_ambiguous`; `evaluation_pending` bleibt nur als historische T04-Konstante), `authorization ∈ {auto_evaluated, not_applicable}`, ein bounded `files[]` mit genau `{filename, sha256, mime_type, chars, coverage, run_id}` (`coverage ∈ {full, truncated}`), `used_for_classification` **immer** `false`, `classifier_revision` **immer** `null`.
