...
* **Finales `attachment_evaluation` im DraftManifest (FR-15 / MD-E2-T01)** ([`scripts/core/attachment_reclassification.py`](../scripts/core/attachment_reclassification.py)):
  ...
  * `classifier_revision = compute_classifier_revision(rules_fingerprint, sortierte konsumierte Hashes)`; `rules_fingerprint` bindet die aktiven Projekt-/Topic-Kataloge plus `CLASSIFIER_RULES_VERSION`. Reihenfolge ist irrelevant; Caller-, Mail- und Manifest-Werte können die Revision nicht setzen.
