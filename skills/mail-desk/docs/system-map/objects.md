...
* **Finales `attachment_evaluation` im DraftManifest (FR-15 / MD-E2-T01/T02/T03/T04)** ([`scripts/core/attachment_reclassification.py`](../scripts/core/attachment_reclassification.py)):
  * Schmaler Orchestrierungs-Seam `install_draft_attachment_evaluations(...)`: nach der bestehenden Klassifikation erhält **jedes** Draft-Item genau ein additives `attachment_evaluation` in der gebundenen MD-E1-Form (sechs Felder).
  * Klares Item → `not_needed`/`classification_clear`/`not_applicable` ohne Rohabruf, Auswertung oder Neuklassifikation. Deaktiviert (`evaluate_attachments: false`) → `skipped`/`evaluation_disabled`/`not_applicable`, ohne Fetch/Reklassifikation. Fortbestehende Mehrdeutigkeit → `completed`/`still_ambiguous`. MD-E1-Fehler/no-op → das bounded staged Objekt. Alle diese Ausgänge bleiben `used_for_classification: false`, `classifier_revision: null`.
  * Nur ein unklares Item ruft `attachment_evaluate` genau einmal auf; ein validierter `ready`-Handoff wird genau einmal als getrenntes `untrusted_external` (`prompt_content`, sichtbare Truncation-Marker) an die bestehenden Classifier-Regeln übergeben. Eine erfolgreiche, eindeutige Neuklassifikation → `completed`/`classification_clear`/`auto_evaluated`, `used_for_classification: true` und 64-Hex-`classifier_revision`; sie darf nur `kind`, `id`, kataloggebundene Unterentscheidungen und `needs_reply` neu ableiten und keine Ziele erfinden.
  * **T02-Härtung:** Jede bounded MD-E1-Fehler-/No-Op-Ursache bleibt item-lokal in Review/`INBOX` (`lock_unavailable`, `policy_blocked`, `quota_exceeded`, `fetch_failed`, `extraction_failed`, `handoff_invalid`, `classification_clear`, `no_attachments`, `no_allowed_attachments`, `still_ambiguous`). Identitäts-/Quellen-Pairing-Fehler sind Bindungsfehler (`handoff_invalid`). Der `ready`-Handoff wird vor der Klassifikation mit `validate_attachment_handoff` gegen die vertrauenswürdige Identität, den Vorab-Entscheid und das kanonische Anhangs-Inventar revalidiert; die konsumierten Hashes müssen eindeutige lowercase 64-Hex sein und exakt den staged `files[]` entsprechen. Fortbestehende Mehrdeutigkeit erhält `completed`/`still_ambiguous` mit `auto_evaluated` und unveränderten sicheren `files[]` (inkl. Coverage/Truncation). Unerwartete Backend-/Programmiervertragsfehler (nicht-Mapping-Ergebnis/-Staged/-Reclassifier, unerwartete Status/Reason/Authorization/Files) schlagen fail-loud über `AttachmentReclassificationContractError` fehl und werden nie als `fetch_failed`/`still_ambiguous` umetikettiert. Langlebige `files[]` bleiben die kanonischen sechs sicheren MD-E1-Felder.
  * `classifier_revision = compute_classifier_revision(rules_fingerprint, sortierte konsumierte Hashes)`; `rules_fingerprint` bindet die normalisierten ASTs der geordneten aktiven Regelmodule (`classifier.py`, `matching/ambiguity.py`, `matching/date_parser.py`, `matching/project_matching.py`, `matching/topic_matching.py`) plus die aktiven Projekt-/Topic-Kataloge und `CLASSIFIER_RULES_VERSION`. Die Reihenfolge der konsumierten Anhangs-Hash-Eingaben ist irrelevant, weil die Hashes dedupliziert und sortiert werden; die **Listenreihenfolge der Projekt-/Topic-Kataloge bleibt dagegen absichtlich im Fingerprint erhalten und ist revisionsrelevant**, weil das Root-Projekt-Matching in `matching/project_matching.select_project_match` und das Root-Topic-Matching in `matching/topic_matching.select_topic_match` geordnete First-Match-Schleifen (`for proj in (projects or []) … break`, `for top in (topics or []) … break`) verwenden. Code-Regel- und Katalog-Änderungen bewegen die Revision, Kommentare/Formatierung/Host-Pfade nicht. Caller-, Mail- und Manifest-Werte können die Revision nicht setzen. Seit **FR-13/MD-M1 (T01–T04, abgeschlossen)** sind genau diese fünf Module die kontrahierte Facade (`classifier.py`, ≤ 813 Zeilen) plus vier Matching-Owner; die erste MD-M1-Ausgabe rotierte vorhandene `classifier_revision`-Werte einmalig (genehmigt).
  * **Deterministische Idempotenz:** `derive_evaluation_run_id` leitet aus Account/Folder/Envelope/Message einen sicheren, PII-freien Run-ID je Nachricht ab; ein optionaler Caller-`attachment_run_id` dient nur als Basis-Namespace. Der zweite Default-Lauf erreicht MD-E1 `already_fetched` ohne zweiten Anhang; kein zweiter Cache/Index.
  * **T04-Paketabnahme (FR-15 geschlossen):** [`tests/test_batch_runner_mde2_acceptance.py`](../tests/test_batch_runner_mde2_acceptance.py) beweist in einem einzigen hermetischen realen Pfad (`run_draft_mode` mit echtem `draft_manifest`, echter `classify_email`-Regelbasis und echtem MD-E1 `attachment_evaluate`; Fakes nur an Message-Listing/Full-Read/Raw-RFC-822) die vollständige Kette bis zum persistierten Projekt-`DraftManifest`: genau eine `untrusted_external`-Neuklassifikation, bounded `attachment_evaluation` mit `used_for_classification: true`, 64-Hex-`classifier_revision` aus Regel-Fingerprint plus tatsächlichem Anhangs-SHA-256 und null Leck von Roh-/`prompt_content`-/Pfad-/Capability-/Receipt-Inhalten. Die negative Matrix, Mischfall-Isolation, No-Double-Fetch und sichtbare Truncation sind in den fokussierten T01–T03-Suiten abgedeckt.
* **Opt-in `inspect`-`manifest_proposal` (FR-15 / MD-E2-T03)** ([`scripts/core/modes/inspect.py`](../scripts/core/modes/inspect.py)):
  * `inspect` bleibt ohne `evaluate_attachments`/`propose_manifest` rein lesend und liefert keine Evaluation. `evaluate_attachments: true` impliziert den top-level `manifest_proposal`; der Inspektor baut das Manifest über `draft_manifest(ordered_emails, …, source_sink=…)` (initiale Klassifikation zuerst) und ruft `install_draft_attachment_evaluations(...)` mit denselben Bindungen wie `draft` auf. Jedes Proposal-Item trägt genau ein finales `attachment_evaluation`.
  * `propose_manifest: true` bei deaktivierter Auswertung installiert je Proposal-Item genau ein `skipped`/`evaluation_disabled`/`not_applicable` ohne Rohabruf, Auswertung oder Neuklassifikation. Ein gemischter Batch (klar/geklärt/weiterhin mehrdeutig/bounded failed) bewahrt Reihenfolge und item-lokale Isolation.
  * Ein ausführbares Batch-Manifest entsteht nur bei explizitem `manifest_file`; `output_file` bleibt das normale Inspect-Ergebnis. Der langfristige Inspect-/Proposal-Output trägt keinen Rohinhalt, `prompt_content`, absolute Pfade oder Capability. Run-ID-/`already_fetched`-Idempotenz und Zero-Mutation werden unverändert von T01/T02 geerbt.
