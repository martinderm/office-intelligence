...
* **Finales `attachment_evaluation` im DraftManifest (FR-15 / MD-E2-T01/T02/T03/T04)** ([`scripts/core/attachment_reclassification.py`](../scripts/core/attachment_reclassification.py)):
  ...
  * `classifier_revision = compute_classifier_revision(rules_fingerprint, sortierte konsumierte Hashes)`; `rules_fingerprint` bindet in MD-M1-T01 die normalisierten ASTs der geordneten aktiven Regelmodule (`classifier.py`, `matching/ambiguity.py`, `matching/date_parser.py`; MD-M1-T02/T03 erweitern die Menge) plus die aktiven Projekt-/Topic-Kataloge und `CLASSIFIER_RULES_VERSION`. Die Reihenfolge der konsumierten Anhangs-Hash-Eingaben ist irrelevant, weil die Hashes dedupliziert und sortiert werden; die **Listenreihenfolge der Projekt-/Topic-Kataloge bleibt dagegen absichtlich im Fingerprint erhalten und ist revisionsrelevant**, weil das Root-Projekt-/Topic-Matching in `classifier.py` geordnete First-Match-Schleifen (`for proj in (projects or []) … break`) verwendet. Code-Regel- und Katalog-Änderungen bewegen die Revision, Kommentare/Formatierung/Host-Pfade nicht. Caller-, Mail- und Manifest-Werte können die Revision nicht setzen.
