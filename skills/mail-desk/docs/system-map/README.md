...
│  ├─ attachment-quarantine-index.json (Schema 1, 16 kanonische Felder)  │
...
| :--- | :--- | :--- |
| **Quarantäne-Engine** | [`scripts/core/attachment_quarantine_index.py`](../scripts/core/attachment_quarantine_index.py)<br>[`scripts/core/attachment_fetch.py`](../scripts/core/attachment_fetch.py)<br>[`scripts/core/attachment_extract.py`](../scripts/core/attachment_extract.py) | Schema 1 Index, deterministische `attachment_id`, SHA-256 Disk-Verifikation, Symlink- & Reparse-Point-Blockade. |
...
| :--- | :--- | :--- |
| **Nomen** (Struktur & Zustand) | [`objects.md`](objects.md) | `attachment-quarantine-index.json`, `.quarantine-inventory.json`, `final-location-index.json`, Dossiers, DTOs. |
| **Verben** (Ablauf & Transformation) | [`processes.md`](processes.md) | Batch-Lifecycles (`draft` → `execute` → `verify`), Quarantäne-Ablauf, Reconcile, Himalaya-Intake. |
...
4. **Schema 1 Quarantäne-Integrität:** Der Quarantäne-Index erzwingt exakt 16 kanonische Felder und deterministische 64-Hex `attachment_id`s. Unbekannte Root- oder Item-Felder führen zum sofortigen Abbruch (`AttachmentIndexSchemaError`).
5. **Fail-Closed Drift-Abbruch:** Jede Diskrepanz zwischen physischer Datei, Inventar-Hash und Index stoppt sofort als `AttachmentIndexDriftError`.
