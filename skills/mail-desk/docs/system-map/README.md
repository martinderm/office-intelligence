...
> **Ziel**: Kompakte, zitierbare Architekturkarte der Mail-Desk-Engine (reproduzierbare Git-Index-Metrik via `git ls-files`: 117 getrackte Dateien; 58 getrackte Dateien unter `scripts/`, davon 47 unter `scripts/core`; 44 Testmodule; 742 Tests) zur Vermeidung von Context-Bloat und Attention Drift bei Refactorings, Quarantäne-Erweiterungen und Bugfixes.
...
│  ├─ Core Domain: scripts/core/ (47 getrackte Dateien)                  │
│  │   ├─ himalaya.py (CLI-Adapter, UNC-Normalisierung, Fail-Fast)       │
│  │   ├─ classifier.py (Facade + matching/ project, date & ambiguity)   │
...
| **Klassifikation & Triage** | [`scripts/core/classifier.py`](../scripts/core/classifier.py)<br>[`scripts/core/matching/date_parser.py`](../scripts/core/matching/date_parser.py)<br>[`scripts/core/matching/ambiguity.py`](../scripts/core/matching/ambiguity.py)<br>[`scripts/core/matching/project_matching.py`](../scripts/core/matching/project_matching.py)<br>[`scripts/core/attachment_policy.py`](../scripts/core/attachment_policy.py) | `classifier.py` bleibt die kompatible Facade für Katalog-Matching und Signalanalyse; die kanonischen Owner `matching/date_parser.py` (Datums-Parsing), `matching/ambiguity.py` (Ranking/Unique-Choice, Cross-Kind-Conflict, Fallback) und `matching/project_matching.py` (Artefakt-Primitive, Projekt-/Workpackage-/Task-/Deliverable-/Milestone-Auflösung, Projekt-Evidenz und der Root-Match-Owner `select_project_match`) sind per Objektidentität an die Facade gebunden (FR-13/MD-M1-T01/T02). |
...
High/Medium-Konfidenz, Ergebnisform); `classify_email` leitet den Root-Projektentscheid
über ihn. [`scripts/core/classifier.py`](../scripts/core/classifier.py) bleibt die
kompatible Facade, re-exportiert jeden verschobenen Callable per Objektidentität und leitet
weiterhin Topic-/Operation-Ranking, Parent-Topic-Fallback und Cross-Kind-Conflict über die
Ambiguity-Owner-Callables. Keine Projekt-Implementierung bleibt als Kompatibilitätskopie in
der Facade; die Matching-Module importieren `classifier.py` nie (kein Zyklus, keine
...
T01/T02-Quelle-Menge `classifier.py`, `matching/ambiguity.py`, `matching/date_parser.py`,
`matching/project_matching.py` (host-pfad-invariant, Kommentar-/Formatierungs-invariant,
fail-closed ohne Partial-Digest).

**Noch offen (Zukunft):** **MD-M1-T03** extrahiert `matching/topic_matching.py`; erst
danach wächst die gebundene Quelle-Menge auf fünf Module und die Facade auf ≤ 813 Zeilen.
MD-M2 und die Quarantäne-Paketierung bleiben out of scope.
