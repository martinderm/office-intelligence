...
3. **Workspace-Lock Ownership:** Schreibende Skripte erfordern eine verifizierte Lease via `require_workspace_lock()`. Legacy-Bypässe (`allow_legacy=True`/`WORKSPACE_LOCK_ALLOW_LEGACY`) sind normativ strikt verboten. **Bekannter Drift:** Attachment-Fetch-/Extraktionspfade werten den Env-Bypass derzeit noch aus; seine Schließung ist blockierende Voraussetzung für FR-15/MD-E1 (Details: [`effects.md`](effects.md) §3).
