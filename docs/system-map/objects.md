...
* **Staged `attachment_evaluation` (FR-15/MD-E1-T04)**: `attachment_evaluate` gibt exakt `{"attachment_evaluation": {…}}` zurück mit `status ∈ {completed, not_needed, skipped, failed}`, bounded `reason`, `authorization ∈ {auto_evaluated, not_applicable}`, bounded `files[]`, `used_for_classification` **immer** `false` und `classifier_revision` **immer** `null`. T04 ist ein Skeleton: der Pre-T05-Ausgang für einen zulässigen Anhang ist `skipped`/`evaluation_pending`/`auto_evaluated`/`files: []`; Fetch, Extraktion, Handoff und `DraftManifest`-Installation bleiben MD-E1-T05. L2-Detail: [`../../skills/mail-desk/docs/system-map/objects.md`](../../skills/mail-desk/docs/system-map/objects.md) §6.
