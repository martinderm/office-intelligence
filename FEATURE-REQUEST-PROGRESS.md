...
- `FR-11 / MD-Q3` — Disposition, Promotion-Link und sichere Bereinigung ist umgesetzt und gehärtet:
  ...
  - Persistiertes Apply-/Recovery-Journal (`data/mail-desk/attachment-discard-journal.json`):
    ...
    - Strenge Verifikation bei bereits entferntem Indexeintrag: Ist ein Anhang nicht mehr im Quarantäneindex, wird die Anfrage nicht blind akzeptiert. Der Apply-Request wird kanonisch aus dem Journal rekonstruiert, das Receipt dagegen verifiziert und ein exakter Match (`attachment_id`, `decision_id`, Receipt-Hash, Zustand `completed` oder `index_updated`) erzwungen.
  ...
  - Review-Verifikation: 24 fokussierte MD-Q3-Tests (inkl. aller adversarieller Tests für manipulierte Zustände, unbekannte Felder, Scope-Drift, Transition-Invarianten, echten Inventarfehlern mit End-to-End-Recovery und Idempotenz), 50 Quarantäne-Tests (MD-Q1–Q3), vollständige Mail-Desk-Suite (514 Tests grün), Compileall, Skill-Catalog-Validierung und `git diff --check` sauber.
