...
- `FR-11 / MD-Q3` — Disposition, Promotion-Link und sichere Bereinigung ist umgesetzt und gehärtet:
  ...
  - Persistiertes Apply-/Recovery-Journal (`data/mail-desk/attachment-discard-journal.json`):
    ...
    - Vollständige Feld-Konsistenzprüfung:
      ...
      - `status: "failed"` erfordert nicht-leere `failure_stage`, ein valides `error`-Dictionary und verbietet `state == "completed"` (`state != "completed"`). Ein fehlerhafter Eintrag wird niemals als idempotenter Erfolg gewertet. Zudem muss der letzte History-Eintrag zwingend ein valider Failure-Eintrag sein (`transition == "failed"`, `status == "failed"`, `stage == failure_stage`, kanonische Übereinstimmung von `error`); nachträglich gesetztes `status: "failed"` ohne Failure-History oder Drift zwischen Top-Level und History werden fail-closed abgewiesen (`RecoveryJournalCorruptedError`).
      - Bei `status: "in_progress"` oder `"completed"` darf der letzte History-Eintrag kein unbehandelter Failure-Eintrag sein.
  ...
  - Review-Verifikation: 29 fokussierte MD-Q3-Tests (inkl. aller adversarieller Tests für manipulierte Zustände, nachträgliches `failed` ohne Failure-History, `failure_stage`/`error`-Drift, verkürzte Historien, widersprüchliche Status/State-Lagen, size_bytes <= 0, unbekannte Felder, Scope-Drift, Transition-Invarianten, echten Inventarfehlern mit End-to-End-Recovery und Idempotenz), 55 Quarantäne-Tests (MD-Q1–Q3), Compileall, Skill-Catalog-Validierung und `git diff --check` sauber.
