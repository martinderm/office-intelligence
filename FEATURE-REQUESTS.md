...
| `FR-17` | 🟡 teilweise; MD-R1, MD-R2, MD-R6 und MD-R7 abgeschlossen und paketabgenommen (katalogtreues Routing, DNR-gegatete Sibling-Kohärenz, begrenzte Client-Deadlines, Verify-Scope/Provenienz/Evidence; 871 Tests grün, Reviews approve) | B-1/B-2/B-8/B-9/B-10 behoben; `classifier_revision` rotiert (MD-R1, MD-R2 — je genau einmal, genehmigt); `evaluate_do_not_route_signal` als kanonischer DNR-Owner, `evaluate_thread_sibling_do_not_route` für Siblings, `overall_deadline_seconds` mit fail-closed Validierung, Subset-Verify-Scope mit Runner-Provenienz und kanonischem Evidence-Fallback | `MD-R3` |
