...
| `FR-04` | 🟡 in Arbeit — FR-04a/b/c abgeschlossen | Ja: kataloggestützter Dossier-Manifest-, Review/Apply- und Synthese-Handoff-Pfad | `batch-dossier.json` bereitet einen begrenzten Inspect-Auftrag vor; `dossier_apply` führt nur einen separat geprüften, hashgebundenen Routingauftrag über Execute→Verify aus; `dossier_synthesis` erzeugt daraus nur einen quellengebundenen, reviewbaren LLM-Arbeitsauftrag. Cloud-/Task-Handover folgt getrennt in FR-04d. |
...
**Status:** 🟡 In Arbeit. FR-04a ist abgeschlossen: `dossier` löst ausschließlich eine exakte Projekt-ID mit Status `active` oder ohne Status (bei v3-Root-Projekten regulär und routingfähig) aus `projects.json` auf und erzeugt lokal einen begrenzten, reviewbaren `batch-dossier.json`-Inspect-Folgeauftrag. Der Modus ist mailbox-read-only/non-executing, aber die lokale Ausgabe benötigt den normalen Workspace-Lock; freie Query sowie Execute, Synthese, Cloud- und Task-Schritte bleiben ausgeschlossen. FR-04b ist abgeschlossen: `dossier_apply` akzeptiert nach separater Human Review nur einen SHA-256-gebundenen kanonischen Execute-Request für exakt dieses Projekt, prüft vor jeder Mutation sämtliche INBOX-/Decision-/Katalogordner-Grenzen und delegiert dann ausschließlich an den vorhandenen Execute→Verify-Pfad. Fehl- und Teilfehler bleiben fail-closed und erhalten Approval-Receipt, Execute-Summary, FR-06-Telemetrie und Handoff zur erneuten Review. FR-04c ist abgeschlossen: `dossier_synthesis` validiert einen erfolgreichen, hash-gebundenen Apply-/Verify-Snapshot und exakt den kanonischen FR-06-Handoff gegen jede Execute-/Verify-Nachrichtenquelle. Er erzeugt nur einen reviewbaren, EVID-verankerten LLM-Arbeitsauftrag; Target-Auswahl, LLM-Aufruf und jede Wissens-, Cloud- oder Task-Mutation bleiben ausgeschlossen.

**Verbleibend:** FR-04d Übergabe an `cloud-atlas` und `task-desk`.
